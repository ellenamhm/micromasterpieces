...
points=[
    ...
    ['about.png','about',true,true,[-0.25,0,500,-0.14,0.06]],
    ['team.png','team',false,false,[0.25,0,1000,0.12,-0.05]],
    ['portfolio.png','portfolio',true,false,[-0.18,0,1500,-0.14,0.25]],
    ['contact.png','contact',false,true,[0.2,0,1990,0.16,0.12]]],
dimensions={
fov:500,
totalHeight:2190,
...
function render(){
    ...
    for(var x=0;x<layers.length;x++){
        ...
        if(layer.render&&layer.width>0&&context.globalAlpha>0){
            switch(layer.type){
                case'image':context.drawImage(layer,layer.coords[0],layer.coords[1],layer.width,layer.height);
                    break;
...
function drawPoint(context,point){
	...
	context.globalAlpha=1;
	context.fillStyle=point.light?'#fff':'#fff';
	context.beginPath();
	context.moveTo(x,y);
	...
	function positionLayer(layer,offsetZ,tilt){var scale=dimensions.fov/(dimensions.fov+ layer.baseCoords[2]+ offsetZ);layer.render=scale>0;switch(layer.type){case'image':layer.width=dimensions.width*scale*layer.baseScale;layer.height=dimensions.height*scale*layer.baseScale+(layer.baseCoords[3]?tilt*0.2:0);break;case'point':layer.width=dimensions.points.width*scale*dimensions.pixelRatio;layer.height=dimensions.points.width*scale*dimensions.pixelRatio*dimensions.points.yAngle;break;case'pattern':case'gradient':layer.width=dimensions.width;layer.height=dimensions.height;break;}
layer.coords=[dimensions.halfWidth-(layer.width/2)+(layer.baseCoords[0]*dimensions.width*scale),dimensions.halfHeight-(layer.height/2)+(layer.baseCoords[1]*dimensions.height*scale)+(tilt*scale)];if(layer.type==='point'){layer.coords[2]=dimensions.halfWidth-(layer.width/2)+(layer.baseCoords[3]*dimensions.width*scale);layer.coords[3]=dimensions.halfHeight-(layer.height/2)+(layer.baseCoords[4]*dimensions.height*scale)+(tilt*scale);}
...
function clickPoint(event){var target=getPointTarget(event);if(target!==false){animate=false;scrollTo(target.targetLayer?target.targetLayer:target.modal,target.modal);}}
